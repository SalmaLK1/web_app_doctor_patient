...
<body>
    ...
    if ($con) {
        // Requête pour récupérer les spécialités distinctes des médecins
        $sql_specialites = "SELECT DISTINCT Speciality FROM Users WHERE Role = 'medecin'";
        $result_specialites = $con->query($sql_specialites);

        // Vérifier si des spécialités sont disponibles
        if ($result_specialites->num_rows > 0) {
            // Parcourir les spécialités et les ajouter comme options
            while($row_specialite = $result_specialites->fetch_assoc()) {
                $specialite = $row_specialite['Speciality'];
                echo "<option value='$specialite'>$specialite</option>";
            }
        } else {
            echo "<option disabled>Aucune spécialité disponible</option>";
        }
    ...
    <table>
        ...
        if ($con) {
            $sql = "SELECT * FROM Users WHERE Role = 'medecin'";
            ...
            if (isset($_GET['speciality']) && !empty($_GET['speciality'])) {
                $specialite = $_GET['speciality'];
                $sql .= " AND Speciality = '$specialite'";
            }
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<tr><td colspan='5'>Aucun médecin trouvé</td></tr>";
            }
            $con->close();
        } else {
